...
\begin{figure}
  ...
  \caption{The rapidity plateau and SSE compared between default PYTHIA 8.315 settings, the \texttt{finalTwo} parameter tune plotted in Figure \ref{fig:dNdysse-goodtune}, and a new parameter tune with $\texttt{stopMass} = 0.1$, $\texttt{stopSmear} = 0.4$, and $\texttt{probRevertFinal} = 0.3$. By introducing \texttt{probRevertFinal}, a small improvement in SSE is possible while maintaining the flatter rapidity plateau.}
...
Through experimentation with all four parameters, the combination $\texttt{stopMass} = 0.1$, $\texttt{stopSmear} = 0.4$, and $\texttt{probRevertFinal} = 0.3$ was found to give a similar improvement in the rapidity plateau. However, the SSE using these parameters is more or less identical to the SSE obtained without \texttt{probRevertFinal}. In general, there was no combination of parameters found that gave a better SSE using \texttt{probRevertFinal} than what was possible without \texttt{probRevertFinal}.

...
\begin{algorithm}
  ...
  \begin{algorithmic}
    \Procedure{Fragment}{$E_\text{CM}, \text{flav}(q_\text{end}^+), \text{flav}(q_\text{end}^-)$}
      \State initialise event record $event$
      \State $p_x(q_\text{end}^+) \gets 0$
      \State $p_y(q_\text{end}^+) \gets 0$ 
      \State $p_x(q_\text{end}^-) \gets 0$ 
      \State $p_y(q_\text{end}^-) \gets 0$ 
      ...
      \Loop
        ...
        \State $\vec{p}_\perp(q_\text{break}^\pm) \gets$ transverse momentum selected according to \eqref{eq:schwinger-mechanism} \Comment{Select transverse momentum}
        \State $\vec{p}_\perp(q_\text{break}^\mp) \gets -\vec{p}_\perp(q_\text{break}^\pm)$
        \State $\vec{p}_\perp \gets \vec{p}_\perp(q_\text{end}^\pm) + \vec{p}_\perp(q_\text{break}^\pm)$
        \State $\vec{p}_\perp(q_\text{end}^\pm) \gets \vec{p}_\perp(q_\text{break}^\mp)$
        \Repeat \Comment{Select string break flavour and new hadron species, mass, and $z$ fraction}
          \State $\text{flav}(q_\text{break}^\pm) \gets \textsc{flavSel}(q_\text{end}^\pm)$
          \State $\text{flav}(q_\text{break}^\mp) \gets \text{anti}(\text{flav}(q_\text{end}^\pm))$
          \State $\text{species} \gets \textsc{combine}(\text{flav}(q_\text{end}^\pm), \text{flav}(q_\text{break}^\pm))$
          \State $m \gets \textsc{msel}(\text{species})$
          \State $m_\perp \gets \sqrt{m^2 + p_x^2 + p_y^2}$
          \State $z \gets \textsc{zfrag}(\text{flav}(q_\text{end}^\pm), \text{flav}(q_\text{break}^\pm), m_\perp)$
          \State $\text{flav}(q_\text{end}^\pm) \gets \text{flav}(q_\text{break}^\mp)$
        \Until {$\textsc{combine}$ succeeds}        
        \If {$\sqrt{p^+ p^-} \leq \textsc{wmin}(q_\text{end}^+,q_\text{end}^-, q_\text{break}^+,q_\text{break}^-)$}
          \State $\textbf{break}$
        \EndIf      
        ...
        \State $E \gets p^+ + p^-$
        \State $p_{z} \gets p^+ - p^-$
        \State $\text{append new hadron with } (\text{species}, m, \vec{p}_\perp, p_z, E) \text{ to event}$
      \EndLoop
      \State $\textsc{finaltwo}(\text{event}, q_\text{end}^+,q_\text{end}^-, q_\text{break}^+,q_\text{break}^-)$
    \EndProcedure
  \end{algorithmic}
\end{algorithm}

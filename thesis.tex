...
Figures \ref{fig:dNdy-probrevertfinal} and \ref{fig:sse-probrevertfinal} show how the rapidity plateau and hadrochemistry SSE vary as \texttt{probRevertFinal} varies from 0 to 1. As expected, increasing \texttt{probRevertFinal} decreases the SSE up to a point. However, if increased beyond a value of approximately $\texttt{probRevertFinal} = 0.4$, the SSE begins to increase again, and a bias is introduced towards heavier hadrons in the final two. It is not immediately obvious why this is the case. A more thorough investigation could be carriedout, but since the overall performance of this new parameter is quite poor (especially compared to the accordion model introduced in chapter \hyperref[chap:accordion]{5}), there are better uses of time.

While there is some worsening of the rapidity plateau as \texttt{probRevertFinal} is increased (since reverting the final break on average increases the energy available for the final two), the effect is quite mild. Through experimentation with all four parameters, the combination $\texttt{stopMass} = 0.1$, $\texttt{stopSmear} = 0.4$, and $\texttt{probRevertFinal} = 0.3$ was found to give a good improvement in the rapidity plateau with a lessened effect on SSE compared to the earlier retune, as shown in Figure \ref{fig:dNdysse-goodtunerevert}.

\section{Limitations}
The \texttt{probRevertFinal} parameter successfully provides more parameter space and allows for Lorentz covariance to be restored further. However, the rapidity plateau in Figure \ref{fig:dNdysse-goodtunerevert} is not as flat as that of the JETSET tune in Figure \ref{fig:dNdy-different-tunes}, and the hadrochemistry SSE is enough for noticeable distortions in particle spectra across the string. The shape of the $\Delta y$ distribution is also different between regular and final hadrons. Indeed, the manifest breaking of Lorentz covariance in the structure of the algorithm means that restoring Lorentz covariance by obtaining a flat rapidity plateau and a negligible hadrochemistry SSE is impossible without using a different model to conserve energy in string fragmentation. In the next section, we describe a new model of string fragmentation that enforces energy conservation in a Lorentz covariant way, restoring Lorentz covariance to the PYTHIA hadronisation algorithm.

...
\begin{figure}
  ...
  \begin{tikzpicture}[scale=0.75]
    ...
    \node[anchor=south east,cyan] at (1,1.1) {$\bar{q}$};
  \end{tikzpicture}
...
\begin{algorithm}
  \begin{algorithmic}
    ...
    \EndProcedure
  \end{algorithmic}
  \caption{The default PYTHIA 8.316 algorithm for $q\bar{q}$ hadronisation.} \label{alg:default}
\end{algorithm}
...
\footnotetext{In general, the plots shown in this thesis are generated using at least 1,000,000 events. Specific numbers are not included for brevity, but the number of events is selected such that the curve is smooth and uncertainties are negligible. As a result, uncertainties are also generally not included in plots.}
...
\begin{figure}
  ...
  \begin{minipage}[t]{0.49\linewidth}
    ...
    \caption{Plots of the rapidity plateau $dN/dy$ as a function of rapidity $y$ in PYTHIA 8.316 across the Monash, JETSET, and Hoeth tunes \protect\footnotemark. Separate plots are shown for the Monash tune in the versions PYTHIA 8.315 and PYTHIA 8.316, between which \texttt{stopMass} was changed from 1.0 to 0.8. This has the effect of making the rapidity plateau slightly more flat. The rapidity plateau for the JETSET tune is not the same as that in Figure \ref{fig:version-plots}. This is because of a bug introduced in PYTHIA 8 that was not present in PYTHIA 6. }
...
Having established the extent and causes of the breaking of Lorentz covariance in PYTHIA hadronisation, we can now move on to exploring solutions to this problem. It would be easiest if we could at the very least tune for a flat rapidity plateau (and ideally restore Lorentz covariance) using the three parameters that control when the final two hadrons are created --- \texttt{stopMass}, \texttt{stopNewFlav}, and \texttt{stopSmear}. Figure \ref{fig:deltay-stopmass} shows that, in the Monash tune, the means of the rapidity spacings along the string can be matched by decreasing \texttt{stopMass} to a very low value, and that the deviation in $\Delta y$ increases linearly with \texttt{stopMass}. Figure \ref{fig:dNdy-stopmass} shows the rapidity plateau for \qty{200}{\giga\electronvolt} $d\bar{d}$ hadronisation for varying values of \texttt{stopMass}, and the effect on the flatness is clear --- as expected, the $dN/dy$ distribution becomes more flat when the means of the $\Delta y$ distributions along the string are more closely aligned, and is almost completely flat for $\texttt{stopMass} = 0.05$.

The issue with simply turning down \texttt{stopMass} (or reducing $W_\text{min}$ in general) is that the bias towards lighter hadrons in the final two hadrochemistry is significantly increased for smaller $W_\text{min}$, and in the extreme case of $\texttt{stopMass} = 0.05$ failures are common enough to cause noticeable slowdowns and pions make up (((VALUE)))\% of all final hadrons. Figure \ref{fig:sse-stopmass} shows how the hadrochemistry SSE varies with \texttt{stopMass} --- worse, there is no value that brings it close to zero.
...
Similar to \texttt{stopMass}, \texttt{stopNewFlav} affects the mean of $W_\text{min}$, but with a more favourable effect on hadronic chemistry since this parameter accounts somewhat for the mass of the final hadrons that will be produced in its effect on $W_\text{min}$. Figure \ref{fig:sse-stopnewflav} shows how the hadrochemistry SSE varies with \texttt{stopNewFlav}. (((COMMENT))).

\begin{figure}
  \centering
  \includesvg[width=0.6\linewidth]{figures/dNdysse_goodtune.svg}
  \caption{}
    ...
    \Procedure{Fragment}{$E_\text{CM}, \text{flav}_{+\text{end}}, \text{flav}_{-\text{end}}$}
      \Repeat
        ...
        \Loop
          ...
          \Until {\textsc{Combine} succeeds}
          \If {$\sqrt{p^+ p^-} \leq \textsc{Wmin}(\text{flav}_{+\text{end}},\text{flav}_{-\text{end}}, \text{flav}_{+\text{break}},\text{flav}_{-\text{pbreak}})$}
            \State $\textbf{break}$
          \EndIf
...
\begin{figure}
  ...
  \begin{minipage}[t]{0.48\linewidth}
    \centering
    \includesvg[width=0.9\linewidth]{figures/dNdy_probrevertfinal.svg}
    \caption{}
  ...
  \begin{minipage}[t]{0.48\linewidth}
    \centering
    \includesvg[width=0.9\linewidth]{figures/sse_probrevertfinal_out.svg}
    \caption{}
...
\begin{figure}
  \centering
  \includesvg[width=0.6\linewidth]{figures/dNdysse_goodtune_revert.svg}
  \caption{}
  \label{fig:dNdysse-goodtunerevert}
\end{figure}

Figures \ref{fig:dNdy-probrevertfinal} and \ref{fig:sse-probrevertfinal} show how the rapidity plateau and hadrochemistry SSE vary as \texttt{probRevertFinal} varies from 0 to 1. As expected, increasing \texttt{probRevertFinal} decreases the SSE, and while there is some worsening of the rapidity plateau (since reverting the final break on average increases the energy available for the final two), the effect is quite mild. Through experimentation with all four parameters, the combination $\texttt{stopMass} = 0.1$, $\texttt{stopSmear} = 0.4$, and $\texttt{probRevertFinal} = 0.3$ was found to give a good improvement in the rapidity plateau with a lessened effect on SSE compared to the same values with $\texttt{probRevertFinal = 0}$, as shown in Figure \ref{fig:dNdysse-goodtunerevert}.
...
Having developed the accordion model and elaborated on its algorithmic implementation in PYTHIA, we can now move on to the results. An ideal selection of \texttt{stopMass} and \texttt{stopSmear} to match distributions of hadron multiplicity with the existing tunes and obtain a flat rapidirty plateau was determined through trial and error. Figure \ref{fig:dNdysse-accordion} shows how the accordion model performs compared to PYTHIA 8.316 defaults with the Monash tune for \qty{200}{\giga\electronvolt} $d\bar{d}$ hadronisation in terms of the rapidity plateau, hadrochemistry SSE, and \texttt{finalTwo} failure rate. The results show a flat rapidity plateau away from the endpoints, and a SSE that is negligible compared to PYTHIA defaults, indicating that Lorentz covariance is preserved by the accordion model.
...
\begin{figure}
  ...
  \begin{minipage}[t]{0.48\linewidth}
    ...
    \caption{The distributions of hadron multiplicity (number of primary hadrons) resulting from \qty{200}{\giga\electronvolt} $d\bar{d}$ hadronisation, compared between PYTHIA 8.316 defaults and the accordion model. The distributions match almost perfectly, demonstrating that the accordion model can be tuned with \texttt{stopMass} to align the multiplicity distributions.}

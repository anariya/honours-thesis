...
I would first like to thank my supervisor Prof. Peter Skands for his invaluable guidance and assistance --- without him to help me get my head around the (very technical) concepts behind the Lund model, and bounce ideas off of when I was stuck, this entire project would have been impossible. I'd also like to thank my parents for keeping a roof over my head in the rare event that I actually slept, and of course, the entire honours physics and astrophysics cohort for being amazing people to trauma bond with!
...
Finally, a special acknowledgement to my cat Pico (Figure \ref{fig:pico}).
...
It was later found that these partons come in two types --- the spin $\frac{1}{2}$ quark, and the spin 1 gluon \cite{ParticleDataGroup:2024cfk}. The existence of the gluon was confirmed in the JADE experiment \cite{JADE:1979rke}, which was notably one of the first uses of Monte Carlo event generators, utilising the JETSET generator (a precursor to PYTHIA) in theanalysis of experimental data. In addition to their electromagnetic charge, quarks and gluons also carry colour charge, which has three components labelled red, green, and blue (with cyan, magenta, and yellow as their anticolour opposites) \cite{Thomson:2013zua,ParticleDataGroup:2024cfk}.
...
\begin{figure}
  ...
  \begin{minipage}[t]{0.48\linewidth}
    ...
    \caption{A zoomed-in spacetime diagram showing $q\bar{q}$ string fragmentation, in a frame where the entire system is moving to the left. In this frame, the string breaks are ordered right to left, allowing for the fragmentation process to be considered iteratively.}
...
\begin{table}
  ...
  \begin{tabular}{l|lll}
    ...
    \texttt{dyOffsetFinalBetween}   & -0.495          & -0.32           & -0.5           \\
    \texttt{dyOffsetFinalNeighbour} & -0.53           & -0.55           & -0.52         
  \end{tabular}
  \caption{The values of the accordion tuning parameters used for the various tunes. The \texttt{stopMass} parameter was tuned to match the hadron multiplicity distributions between the accordion model and the default fragmentation model. The \texttt{dyOffset} parameters were tuned to match the $\Delta y$ distributions along the string and therefore obtain a flat rapidity plateau.}
  \label{tab:accordion-tunes}
\end{table}

\begin{sloppypar} Ideally, equation \eqref{eq:rapidity-spacing} would be derived for fragmentation from both ends of the string. Due to time constraints, we instead introduce two tunable parameters \texttt{StringFragmentation:dyOffsetFinalBetween} and \texttt{StringFragmentation:dyOffsetFinalNeighbour} which add (or subtract) a constant offset to $\Delta y_\text{between}$ (the rapidity spacing between final hadrons) and $\Delta y_\pm$ (the rapidity spacing between final hadrons and their regular neighbours). By tuning these parameters, the $\Delta y$ distributions obtained from the combination of \textsc{zSel} and equation \eqref{eq:rapidity-spacing} can be made to match the $\Delta y$ distribution of regular hadrons exactly. Since rapidity spacings depend only on the mass and $z$ fraction distributions of hadrons, and not on the centre-of-mass energy or other event-specific variables, these parameters only need to be set once for each tune. Table \ref{tab:accordion-tunes} shows selections of these parameters, alongside \texttt{stopMass}, that match the $\Delta y$ and hadron multiplicity distributions. \end{sloppypar}
...
Having developed the accordion model and elaborated on its algorithmic implementation in PYTHIA, we can now move on to the results. An ideal selection of \texttt{stopMass} and \texttt{stopSmear} to match distributions of hadron multiplicity with the existing tunes and obtain a flat rapidirty plateau was determined through trial and error. Figure \ref{fig:dNdysse-accordion} shows how the accordion model performs compared to PYTHIA 8.315 defaults with the Monash tune for \qty{200}{\giga\electronvolt} $d\bar{d}$ hadronisation in terms of the rapidity plateau, hadrochemistry SSE, and \texttt{finalTwo} failure rate. The results show a flat rapidity plateau away from the endpoints, and a SSE that is negligible compared to PYTHIA defaults, indicating that Lorentz covariance is preserved by the accordion model.
...
\begin{figure}
  ...
  \begin{minipage}[t]{0.48\linewidth}
    ...
    \caption{Rapidity plateaus for \qty{200}{\giga\electronvolt} $d\bar{d}$ hadronisation using the accordion model, for the Monash, Hoeth, and JETSET tunes. The \texttt{stopMass} parameter is tuned to a different value for each --- 7.8, 1.5, and 1.25 respectively --- in order to match the hadron multiplicity distributions with PYTHIA defaults and ensure a flat rapidity plateau. Compared to Figure \ref{fig:dNdy-different-tunes}, there is a very significant improvement, and the plateaus are now all flat.}
...
Figure \ref{fig:dNdy-accordion-species} shows the rapidity plateaus in \qty{200}{\giga\electronvolt} $d\bar{d}$ hadronisation for four different species of hadrons (combined with the corresponding antiparticle) --- $\pi^0$, $\Delta^0$, $K^0$, and $p$. Unlike the overall rapidity plateaus, these are not exactly flat, which is again because equation \eqref{eq:rapidity-spacing} does not exactly match the rapidity spacings of regular hadrons. However, compared to Figure \ref{fig:species-specific}, they are significantly flatter than the equivalent plots using PYTHIA defaults.

...
I would first like to thank my supervisor Prof. Peter Skands for his invaluable guidance and assistance --- without him to help me get my head around the (very technical) concepts behind the Lund model, and bounce ideas off of when I was stuck, this entire project would have been impossible.
...
With the requisite background theory covered, this section will go on to describe how the Lund model of string fragmentation is implemented in PYTHIA, and how this implementation performs. Section \hyperref[sec:pythia-frag]{3.1} describes the algorithm used for $q\bar{q}$ hadronisation in PYTHIA 8, owing to the description of the algorithm in the manuals \cite{Bierlich:2022pfr,Sjostrand:2006za,PYTHIAmanualold} as well as inspecting the actual code. Section \hyperref[sec:final-two]{3.2} goes on to describe how the \texttt{finalTwo} procedure works, which creates the final two hadrons to preserve conservation of energy. Finally, Section \hyperref[sec:performance]{3.3} marks the beginning of the original research, and evaluates the Lorentz covariance of the PYTHIA 8 hadronisation algorithm.
...
\begin{figure}
  ...
  \begin{minipage}[t]{0.49\linewidth}
    ...
    \caption{Plots of the rapidity plateau $dN/dy$ as a function of rapidity $y$ in PYTHIA 8.315 across the Monash, JETSET, and Hoeth tunes \protect\footnotemark. Separate plots are shown for the Monash tune in the versions PYTHIA 8.315 and PYTHIA 8.316, between which \texttt{stopMass} was changed from 1.0 to 0.8. This has the effect of making the rapidity plateau slightly more flat. The rapidity plateau for the JETSET tune is not the same as that in Figure \ref{fig:version-plots}. This is because of a bug introduced in PYTHIA 8 that was not present in PYTHIA 6. }
...
This behaviour is visible in Figure \ref{fig:deltay-stopmass} which shows how, for the Monash and JETSET tunes, the mean final rapidity difference $\overline{\Delta y}_\text{final}$ varies for differing values of \texttt{stopMass} (the parameter with the most direct impact on the mean of $W_\text{rem}$). (((clarify notation, use angle brackets))) As expected, $\overline{\Delta y}_\text{final}$ increases as \texttt{stopMass} increases and more kinetic energy is available for the final two. The horizontal dotted lines show the mean rapidity differences between regular hadrons along the string, $\overline{\Delta y}_\text{reg}$. The mean rapidity differences for both regular and final hadrons vary with the tune. This is because the kinematics of both regular and final hadrons are dependent on the various PYTHIA parameters that control fragmentation functions and relative quark/hadron flavour/species probabilities. The dependence is complicated, and an analytic expression is not necessary here.
...
\begin{figure}
  ...
  \begin{subfigure}{0.4\linewidth}
    \centering
    \includesvg[width=\linewidth]{figures/sse_monjet.svg}
  \end{subfigure}
  \hfill
  \begin{subfigure}{0.4\linewidth}
    \centering
    \includesvg[width=\linewidth]{figures/fail_monjet.svg}
  \end{subfigure}
  \caption{Hadrochemistry sum of squared errors and average number of events for each \texttt{finalTwo} failure, in the Monash and JETSET tunes, computed for \qty{200}{\giga\electronvolt} $d\bar{d}$ hadronisation. In the JETSET tune, the hadrochemistry is worse, which corresponds to an increased failure rate of \texttt{finalTwo}. It is evident that the hadrochemistry is quite poor for both tunes.}
  \label{fig:ssefail-monjet}
\end{figure}

The values of the sum of squared errors for the JETSET and Monash tunes are shown in Figure \ref{fig:ssefail-monjet}, alongside the average number of failures per generated event. Evidently, for both tunes, the failure rate of \texttt{finalTwo} is high, and there are notable deviations in hadronic chemistry.
...
The issue with simply turning down \texttt{stopMass} (or reducing $W_\text{min}$ in general) is that the bias towards lighter hadrons in the final two hadrochemistry is significantly increased for smaller $W_\text{min}$, and in the extreme case of $\texttt{stopMass} = 0.05$ failures are common enough to cause noticeable slowdowns and $\pi^0$ is overproduced by $65.8 \pm 0.9 \%$. Figure \ref{fig:sse-stopmass} shows how the hadrochemistry SSE varies with \texttt{stopMass} --- worse, there is no value that brings it close to zero.
...
Similar to \texttt{stopMass}, \texttt{stopNewFlav} affects the mean of $W_\text{min}$, but with a more favourable effect on hadronic chemistry since this parameter accounts somewhat for the mass of the final hadrons that will be produced in its effect on $W_\text{min}$. Figure \ref{fig:sse-stopnewflav} shows how the hadrochemistry SSE varies with \texttt{stopNewFlav}. The dependence is similar to that of Figure \ref{fig:sse-stopmass}, but the SSE increases faster as \texttt{stopNewFlav} is decreased below 2.0, which is because its impact on $W_\text{min}$ is flavour-dependent.
...
Figures \ref{fig:dNdy-probrevertfinal} and \ref{fig:sse-probrevertfinal} show how the rapidity plateau and hadrochemistry SSE vary as \texttt{probRevertFinal} varies from 0 to 1. As expected, increasing \texttt{probRevertFinal} decreases the SSE up to a point. However, if increased beyond a value of approximately $\texttt{probRevertFinal} = 0.4$, the SSE begins to increase again, and a bias is introduced towards heavier hadrons in the final two. It is not immediately obvious why this is the case, and since the overall performance of this new parameter is quite poor (especially compared to the accordion model introduced in chapter \hyperref[chap:accordion]{5}), a more thorough investigation would be relatively fruitless.
...
Figure (((REF))) shows a schematic illustration of how the default PYTHIA 8.315 string fragmentation algorithm implements hadronisation, and Figure (((REF))) shows a similar diagram for the accordion model.
  ...
  \begin{algorithmic}
    \Procedure{FinalTwoAccordion}{$\texttt{event}, \text{newHadronNeeded}, \text{fromPos}, q_{+\text{end}}, q_{-\text{end}}, \\ \quad q_{+\text{break}}, q_{-\text{break}}, \text{species}, m, \vec{p}_\perp, z_{\pm,\text{last}}, m_{\pm,\text{last}}$}
      \If {newHadronNeeded}\Comment{$\pm$ and $\mp$ reflect fromPos selection}
        ...
        \State $\text{species}_+, m_+, \vec{p}_{\perp,+} \gets$ values from \textsc{Combine}, \textsc{mSel}, and Equation \eqref{eq:schwinger-mechanism}
      \Else
        \State $\text{species}_+, m_+, \vec{p}_{\perp,+} \gets \text{species}, m, \vec{p}_\perp$
      \EndIf
      \State $\vec{p}_{\perp,+} \gets \vec{p}_{\perp,+\text{end}} + \vec{p}_{\perp,+\text{break}}$
      \State $\vec{p}_{\perp,-} \gets \vec{p}_{\perp,-\text{end}} + \vec{p}_{\perp,-\text{break}}$
      \Repeat
        \State $\text{species}_- \gets \textsc{Combine}(\text{flav}_{-\text{end}}, \text{flav}_{-\text{break}})$
        \State $m_- \gets \textsc{mSel}(\text{species}_-)$
      \Until {\textsc{Combine} succeeds}
      ...
      \State $\Delta y_- \gets -\ln\Bigr( \frac{z_-}{z_{-,\text{last}}} \frac{m_{-,\text{last}}}{m_-} (1 - z_{-,\text{last}}) \Bigr)$
      \State $\Delta y_\text{between} \gets -\ln\Bigr( \frac{z_-}{z_+} \frac{m_+}{m_-} (1 - z_+) \Bigr)$
...
Upon stopping the fragmentation loop, the final two hadrons are created according to Algorithm \ref{alg:accordion-final-two}. Firstly, a new string break is selected according to the Schwinger mechanism if necessary. The transverse momenta of the final two hadrons can then be calculated. Next, the hadron species and masses are selected according to the quark flavours, in the same manner as the rest of the hadrons from the fragmentation loop.
...
Furthermore, the accordion model has only been implemented for hadronisation of a single $q\bar{q}$ string, with the popcorn model of baryon production switched off. In an actual PYTHIA event, there are dozens of such processes (((REWRITE, ee pp))), which often involve more complicated topologies including gluon kinks, loops, and string junctions. It is not immediately obvious how the property of Lorentz covariance or the global energy conservation of the accordion model would apply to these situations, and further development of analytic models as well as practical implementations is required in order to produce full $p\bar{p}$ collision events using the accordion model.
...
\appendix
\chapter{Derivation of rapidity spacing equation}
